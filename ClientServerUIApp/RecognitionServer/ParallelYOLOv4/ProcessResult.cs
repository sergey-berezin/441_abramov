...
namespace ParallelYOLOv4
{
    public class ProcessResult : System.IDisposable
    {
        ...
        public ProcessResult(IReadOnlyList<RecognitionObject> categories, string imgName, Bitmap bitmap)
        {
            _categoriesCount = new Dictionary<string, int>();
            foreach (var cat in categories)
            {
                if (!_categoriesCount.ContainsKey(cat.ObjName))
                    _categoriesCount[cat.ObjName] = 0;
                _categoriesCount[cat.ObjName]++;
            }
            ImageName = imgName;
            Bitmap = bitmap;
            Categories = categories;
